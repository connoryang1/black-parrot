...
module bp_be_csr
 ...
 #(parameter bp_params_e bp_params_p = e_bp_default_cfg
  ...
  wire mprv_mem_v = (~is_debug_mode | dcsr_lo.mprven)
                    & mstatus_lo.mprv
                    & (mstatus_lo.mpp < `PRIV_MODE_M)
                    & (satp_lo.mode == 4'd8);

  assign trans_info_cast_o.priv_mode      = mprv_mem_v ? mstatus_lo.mpp : priv_mode_r;
  ...
  // CSR 0x082 write: set the NPC for the logical context whose ID is in the upper bits
  ...
  // CSR 0x083 write: remote register write into a target logical context
